...
const buildViewModel = (data = {}) => ({
  ...
  daXacNhan: data.daXacNhan || false,
...
router.get("/xac-nhan-thue-phong", async (req, res) => {
  try {
    const { khachHang, danhSachPhieu } = await layThongTinKhachHang(
    ...
    let daXacNhan = false;
    if (id_phieu) {
      const result = await layDanhSachPhongDaDangKy(
        id_phieu,
        DEFAULT_KHACH_HANG_ID,
        loai_thue,
      );
...
router.post("/xac-nhan-thue-phong/load-phong", async (req, res) => {
  ...
  try {
    ...
    const { daDangKy, giuongTheoPhong, daXacNhan } =
      await layDanhSachPhongDaDangKy(
        id_phieu,
        DEFAULT_KHACH_HANG_ID,
        loai_thue,
      );
  ...
  try {
    ...
    res.render(
      ...
      buildViewModel({
        ...
        giuongTheoPhong: giuongTheoPhong || {},
        daXacNhan: !!daXacNhan,
        success: `Đã xác nhận ${danhSachChon.length} thành công.`,
      }),
    );
  } catch (error) {
    const { khachHang, danhSachPhieu } = await layThongTinKhachHang(
    ...
    const { daDangKy, giuongTheoPhong } = await layDanhSachPhongDaDangKy(
      id_phieu,
      DEFAULT_KHACH_HANG_ID,
      loai_thue,
